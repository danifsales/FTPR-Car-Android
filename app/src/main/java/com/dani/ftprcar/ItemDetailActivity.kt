...
import com.dani.ftprcar.databinding.ActivityItemDetailBinding
import com.dani.ftprcar.model.Item
import com.dani.ftprcar.service.Result
import com.dani.ftprcar.service.RetrofitClient
import com.dani.ftprcar.service.safeApiCall
import com.dani.ftprcar.ui.loadUrl
...
class ItemDetailActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun loadItem() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            val result =
                safeApiCall { RetrofitClient.apiService.getItem(itemId) }
    ...
    private fun deleteItem() {
        CoroutineScope(Dispatchers.IO).launch {
            val result =
                safeApiCall { RetrofitClient.apiService.deleteItem(item.id) }

            withContext(Dispatchers.Main) {
                when (result) {
                    is Result.Error -> {
                        Toast.makeText(
                            thiscom.example.minhaprimeiraapi.ItemDetailActivity,
    ...
    companion object {
        ...
        fun newIntent(
            context: Context,
            itemId: String
        ) = Intent(context, com.example.minhaprimeiraapi.ItemDetailActivity::class.java).apply {
